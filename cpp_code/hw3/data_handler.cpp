...
void data_handler::split_data(void){
    ...
    int valid_data_size = data_array->size()*VALIDATION_SET_PERCENT;
/*
...
*/
    ...
    for(int i=0;i<test_data_size;i++){
        test_data->push_back(data_array->at(i+train_data_size));
    }
    for(int i=0; i<valid_data_size;i++){
        validation_data->push_back(data_array->at(i+train_data_size+test_data_size));
    }
...
vector<data*>* data_handler::get_training_data(void){
    return training_data;
}
vector<data*>* data_handler::get_test_data(void){
    return test_data;
}
vector<data*>* data_handler::get_validation_data(void){
